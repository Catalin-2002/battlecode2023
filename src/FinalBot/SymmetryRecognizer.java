...
public class SymmetryRecognizer {
    ...
    public void hqTest() throws GameActionException{
        updateComms();
    ...
    public void updateComms() throws GameActionException{
        int x = Global.rc.readSharedArray(59);
        if ((x&1) == 1) rotational = false;
        if ((x&2) == 2) horizontal = false;
        if ((x&4) == 4) vertical = false;
        if (Global.rc.canWriteSharedArray(60, 0)){
            int msg = 0;
            if (!rotational) msg |= 1;
            if (!horizontal) msg |= 2;
            if (!vertical) msg |= 4;
            if (msg != x){
                Global.rc.writeSharedArray(60, msg);
            }
        }
    }
